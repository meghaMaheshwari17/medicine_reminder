...
class RefillDetails extends StatefulWidget {
  const RefillDetails({Key? key, required this.medicineName, required this.medicineType,  required this.time, this.date}) : super(key: key);
      ...
      body: Padding(
        ...
        child: Column(children: [
          ...
          Container(
              ...
              child: Text('Refill Reminder Detail',
                  textAlign: TextAlign.center,
                  style: Theme.of(context).textTheme.headlineMedium?.copyWith(color:Colors.green))),
          SizedBox(height: 10.h,),
          MainSection(medicineName: widget.medicineName!, medicineType: widget.medicineType!,),
          ExtendedInfoSection( medicineType:  widget.medicineType!, time: widget.time!,date:widget.date!),
          Spacer(),
          SizedBox(
              width:100.w,
              height:7.h,
...
//  open alert box
        ...
        builder: (context){
          return AlertDialog(
              ...
              actions:[
                ...
                TextButton(onPressed: (){
                  //  delete medicine
                  deleteRefillReminder(widget.medicineName!);
  ...
  void deleteRefillReminder(String medicineName)async{
    final ap = Provider.of<AuthProvider>(context, listen: false);
    int notifyId=await ap.deleteRefillReminderFromFirebase(medicineName);
    print("refill: ${notifyId}");
    await _notifications.removeNotify(notifyId, flutterLocalNotificationsPlugin);
...
class MainSection extends StatelessWidget {
  const MainSection({Key? key, required this.medicineName,required this.medicineType}) : super(key: key);
  final String medicineName;
  final String medicineType;
  @override
  Widget build(BuildContext context) {
...
class ExtendedInfoSection extends StatelessWidget {
  const ExtendedInfoSection({Key? key, required this.medicineType, required this.time, required this.date}) : super(key: key);
  final String medicineType;
  final String time;
  final String date;
  @override
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              ExtendedInfoTab(fieldTitle: "Time", fieldInfo: time)
            ],
          ),
